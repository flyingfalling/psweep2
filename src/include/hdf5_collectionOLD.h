...
struct matrix_props
{
  ...
  void set_datatype( const std::string& dtype )
  {
	...
	matrix_datatype = H5::PredType::NATIVE_DOUBLE;
      ...
      {
	matrix_datatype = H5::PredType::NATIVE_INT;
      }
    else
      {
  ...
  void add_data( const std::vector<std::string>& colnames, const std::vector<std::vector<T>>& toadd )
  {
    ...
    int64_t vec[toadd.size()][toadd[0].size()];
  ...
  std::vector< std::vector<T> > read_whole_dataset()
  {
    ...
    float64_t vec[dims_out[0]][dims_out[1]];
...
struct int_matrix_props
{
  ...
  void new_matrix( const std::string& dsetname, const std::vector<std::string>& _colnames, H5::H5File& f )
  {
    ...
    prop.setChunk(ndims, chunk_dims);
    int64_t fill_val = -666;
    prop.setFillValue( H5::PredType::NATIVE_INT, &fill_val);
      
    //REV: assume its always native double..ugh. Sometimes I'll write ints though. Just do doubles for now...
    //Need to know type?
    dataset =  f.createDataSet( dsetname, H5::PredType::NATIVE_INT,
				dataspace, prop) ;
  ...
  void add_data( const std::vector<std::string>& colnames, const std::vector<std::vector<int64_t>>& toadd )
  {
    //Assume that order of colnames is same. Check that for sanity I guess.
    //Need to extend current dataset.
    H5::DataSpace origspace = dataset.getSpace();
    ...
    //dataset.write( toadd.data(), H5::PredType::NATIVE_DOUBLE, toaddspace, origspace );
    //dataset.write( vec.data(), H5::PredType::NATIVE_DOUBLE, toaddspace, origspace );
    dataset.write( vec, H5::PredType::NATIVE_INT, toaddspace, origspace );
  ...
  std::vector< std::vector<int64_t> > read_whole_dataset()
  {
    H5::DataSpace origspace = dataset.getSpace();
    ...
    std::vector<std::vector<int64_t> > retvect(dims_out[0], std::vector<int64_t>(dims_out[1]) );
    int64_t vec[dims_out[0]][dims_out[1]];
    //dataset.read( retvect.data(), H5::PredType::NATIVE_DOUBLE );
    dataset.read( vec, H5::PredType::NATIVE_INT );
  ...
  std::vector< std::vector<int64_t> > read_row_range( const size_t& startrow, const size_t& endrow)
  {
    ...
    dataset.read( vec, H5::PredType::NATIVE_INT, memspace, origspace );

    std::vector<std::vector<int64_t>> retvect( nrowread, std::vector<int64_t>(ncolread) );
  ...
  {
    fprintf(stdout, "Will enum data set from matrix [%s]:\n\n", name.c_str());
    std::vector<std::vector<int64_t> > ret = read_whole_dataset();
    for(size_t x=0; x<ret.size(); ++x)
      {
	for(size_t y=0; y<ret[x].size(); ++y)
	  {
  ...
  int_matrix_props() // const std::vector<std::string>& _colnames )
  {
...
//REV: This is the problem, we want to do in multiple types...
struct real_matrix_props
{
  std::string name; //we use this as name of thing in H5 file.
  //size_t nrows;
  //size_t ncols;
  ...
  //Fspace is the hyperslab we selected.
  //Mspace is space targeting just the local data to write.
  void add_data( const std::vector<std::string>& colnames, const std::vector<std::vector<float64_t>>& toadd )
  {
    //Assume that order of colnames is same. Check that for sanity I guess.
    //Need to extend current dataset.
    H5::DataSpace origspace = dataset.getSpace();
    ...
    float64_t vec[toadd.size()][toadd[0].size()];
  ...
  std::vector< std::vector<float64_t> > read_whole_dataset()
  {
    H5::DataSpace origspace = dataset.getSpace();
    ...
    std::vector<std::vector<float64_t> > retvect(dims_out[0], std::vector<float64_t>(dims_out[1]) );
    float64_t vec[dims_out[0]][dims_out[1]];
    //dataset.read( retvect.data(), H5::PredType::NATIVE_DOUBLE );
  ...
  std::vector< std::vector<float64_t> > read_row_range( const size_t& startrow, const size_t& endrow)
  {
    ...
    //If this is zero, we read only 1 row???
    size_t nrowread = endrow-startrow+1; //+1 for reading single row
    float64_t vec[ nrowread ][ ncolread ];
    ...
    std::vector<std::vector<float64_t>> retvect( nrowread, std::vector<float64_t>(ncolread) );
  ...
  {
    fprintf(stdout, "Will enum data set from matrix [%s]:\n\n", name.c_str());
    std::vector<std::vector<float64_t> > ret = read_whole_dataset();
    for(size_t x=0; x<ret.size(); ++x)
      {
	for(size_t y=0; y<ret[x].size(); ++y)
	  {
  ...
  real_matrix_props() // const std::vector<std::string>& _colnames )
  {
    //REV: Need to have a file created already.
...
//REV: Make it so that we can also save int64?
